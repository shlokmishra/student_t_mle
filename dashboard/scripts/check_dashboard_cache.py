...
def main() -> None:
    ...
    check(results, "sampler final verdict cache exists", not sampler_verdicts.empty, f"rows={len(sampler_verdicts)}")
    check(results, "sampler diagnostic coverage cache exists", not sampler_coverage.empty, f"rows={len(sampler_coverage)}")
    ...
    pages = [
        ...
        Path("dashboard/pages/1_Posterior_Comparison.py"),
        Path("dashboard/pages/2_Cost_Audit.py"),
        Path("dashboard/pages/3_Model_Validity_Audit.py"),
        Path("dashboard/pages/4_Analysis_Report.py"),
        Path("dashboard/pages/5_KDE_Correctness.py"),
