...
def parse_args() -> argparse.Namespace:
    ...
    backend_choices = ["jax_loop", "jax_scan", "jax_scan_block_z", "numba", "numba_full"]
    parser.add_argument("--backends", nargs="+", default=["jax_loop", "jax_scan", "numba"], choices=backend_choices)
...
def base_params(k: float, n: int, iterations: int, backend: str, args: argparse.Namespace) -> dict:
    return {
        "k": float(k),
        "n": int(n),
        ...
        "store_x_chain": bool(args.store_x_chain),
    }
...
def force_materialized(chain: dict) -> None:
    ...
    if "x_chain" in chain:
        ...
        np.asarray(x_chain)
    if "x_final" in chain:
        np.asarray(chain["x_final"])


def run_case(backend: str, k: float, n: int, iterations: int, warmup_iterations: int, seed: int, args: argparse.Namespace) -> dict:
    print(
        f"START backend={backend} k={k:g} n={n} seed={seed} "
        f"iterations={iterations} warmup_iterations={warmup_iterations}",
        flush=True,
    )
    warmup_params = base_params(k, n, warmup_iterations, backend, args)
    force_materialized(loc_student.run_gibbs(random.PRNGKey(seed + 100_000), 0.0, warmup_params, verbose=False))

    params = base_params(k, n, iterations, backend, args)
    ...
    row = {
        ...
        "block_z_acceptance_rate": float(chain.get("block_z_acceptance_rate", np.nan)),
        "gibbs_pairing_schedule": chain.get("gibbs_pairing_schedule", ""),
        "gibbs_pair_parallel": bool(chain.get("gibbs_pair_parallel", False)),
        "store_x_chain": bool(chain.get("store_x_chain", True)),
        ...
        for k in args.k_values:
            for n in args.n_values:
                for seed in args.seeds:
                    row = run_case(backend, k, n, args.iterations, args.warmup_iterations, seed, args)
                    rows.append(row)
                    print(
