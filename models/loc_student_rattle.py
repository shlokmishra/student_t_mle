...
def _constraint_value(x, mu_star, k, cost_ledger=None):
    y = np.asarray(x) - mu_star
    value = float(np.sum(y / (k + y * y)))
...
def _constraint_grad(x, mu_star, k, cost_ledger=None):
    y = np.asarray(x) - mu_star
    denom = k + y * y
    if cost_ledger is not None:
...
def _constraint_gram(x, mu_star, k, cost_ledger=None):
    if cost_ledger is not None:
        cost_ledger.inc("gram_evals")
    grad = _constraint_grad(x, mu_star, k, cost_ledger=cost_ledger)
    return float(np.dot(grad, grad))


def _constraint_grad_derivative(x, mu_star, k):
    y = np.asarray(x) - mu_star
    denom = k + y * y
    return -2.0 * y * (3.0 * k - y * y) / (denom * denom * denom)


def _grad_log_gram_half(x, mu_star, k, cost_ledger=None):
    if cost_ledger is not None:
        cost_ledger.inc("gram_grad_evals")
    grad = _constraint_grad(x, mu_star, k, cost_ledger=cost_ledger)
    grad_deriv = _constraint_grad_derivative(x, mu_star, k)
...
def _init_diag(x0, mu_star, k, proposals, cost_ledger=None):
    residual0 = abs(_constraint_value(x0, mu_star, k, cost_ledger=cost_ledger))
    return {
...
def _project_to_manifold_newton(
    ...
    residual = abs(_constraint_value(x_trial, mu_star, k, cost_ledger=cost_ledger))
    if residual <= tol:
        return x_trial, True, 0, residual

    if init_strategy == "linearized":
        grad0 = _constraint_grad(x_trial, mu_star, k, cost_ledger=cost_ledger)
        denom0 = float(np.dot(grad0, grad0))
        if np.isfinite(denom0) and denom0 > grad_tol:
            step0 = damping * _constraint_value(x_trial, mu_star, k, cost_ledger=cost_ledger) / denom0
    ...
    for it in range(1, max_iters + 1):
        grad = _constraint_grad(x_cur, mu_star, k, cost_ledger=cost_ledger)
        denom = float(np.dot(grad, grad))
        if not np.isfinite(denom) or denom <= grad_tol:
            return x_cur, False, it, residual

        raw_step = _constraint_value(x_cur, mu_star, k, cost_ledger=cost_ledger) / denom
        alpha = float(damping)
        x_next = x_cur - alpha * raw_step * grad
        next_residual = abs(_constraint_value(x_next, mu_star, k, cost_ledger=cost_ledger))
...
def _project_to_manifold_fixed_direction(
    ...
    """Project x_tilde to c(x)=0 along a fixed direction from the old position.

    Paper RATTLE with M=I and one scalar constraint solves only for scalar theta:
        x_new = x_tilde + direction * theta,
        c(x_new) = 0,
    where direction is grad c(x_old), held fixed throughout Newton iterations.
    """
    if cost_ledger is not None:
        cost_ledger.inc("projection_evals")
    ...
    if not np.all(np.isfinite(direction)) or float(np.dot(direction, direction)) <= grad_tol:
        residual = abs(_constraint_value(x_tilde, mu_star, k, cost_ledger=cost_ledger))
        return x_tilde, False, 0, residual, 0.0

    theta = 0.0
    x_cur = x_tilde
    residual = abs(_constraint_value(x_cur, mu_star, k, cost_ledger=cost_ledger))
    if residual <= tol:
        return x_cur, True, 0, residual, theta

    for it in range(1, max_iters + 1):
        c_val = _constraint_value(x_cur, mu_star, k, cost_ledger=cost_ledger)
        grad = _constraint_grad(x_cur, mu_star, k, cost_ledger=cost_ledger)
        deriv = float(np.dot(grad, direction))
        if not np.isfinite(deriv) or abs(deriv) <= grad_tol:
            return x_cur, False, it, residual, theta

        raw_step = c_val / deriv
        alpha = float(damping)
        theta_next = theta - alpha * raw_step
...
def _grad_potential(x, mu, k, mu_star=None, include_gram_correction=False, cost_ledger=None):
    y = np.asarray(x) - mu
    grad = (k + 1.0) * y / (k + y * y)
    if cost_ledger is not None:
        cost_ledger.inc("gradient_evals")
        cost_ledger.inc("student_grad_evals", np.asarray(x).size)
    if include_gram_correction:
        if mu_star is None:
            raise ValueError("mu_star is required when include_gram_correction=True")
        grad = grad + _grad_log_gram_half(x, mu_star, k, cost_ledger=cost_ledger)
    return grad


def _potential_energy(x, mu, k, mu_star=None, include_gram_correction=False, cost_ledger=None):
    y = np.asarray(x) - mu
    energy = float(0.5 * (k + 1.0) * np.sum(np.log1p((y * y) / k)))
    if cost_ledger is not None:
        cost_ledger.inc("potential_evals")
        cost_ledger.inc("student_logpdf_evals", np.asarray(x).size)
    if include_gram_correction:
        if mu_star is None:
            raise ValueError("mu_star is required when include_gram_correction=True")
        energy += 0.5 * np.log(max(_constraint_gram(x, mu_star, k, cost_ledger=cost_ledger), 1e-300))
    return energy


def _hamiltonian(x, momentum, mu, k, mu_star=None, include_gram_correction=False, cost_ledger=None):
    if cost_ledger is not None:
        cost_ledger.inc("energy_evals")
    kinetic = 0.5 * float(np.dot(momentum, momentum))
    return _potential_energy(x, mu, k, mu_star, include_gram_correction, cost_ledger=cost_ledger) + kinetic
...
def _rattle_trajectory(
    ...
    residual0 = abs(_constraint_value(x, mu_star, k, cost_ledger=cost_ledger))
    diag = {
        "position_projection_failures": 0,
        "momentum_projection_failures": 0,
    ...
    for _ in range(int(num_steps)):
        ...
        if projection_mode == "normal_newton_legacy":
            momentum = momentum - 0.5 * step_size * _grad_potential(
                x, mu, k, mu_star=mu_star, include_gram_correction=include_gram_correction, cost_ledger=cost_ledger
            )
            momentum, ok = _project_momentum(momentum, _constraint_grad(x, mu_star, k, cost_ledger=cost_ledger), grad_tol, cost_ledger=cost_ledger)
            if not ok:
            ...
            if cost_ledger is not None:
                if trajectory_kind == "reverse":
                    cost_ledger.inc("reverse_newton_iters", n_iter)
                else:
                    cost_ledger.inc("forward_newton_iters", n_iter)
            ...
            grad_new = _constraint_grad(x, mu_star, k, cost_ledger=cost_ledger)
            ...
            grad_old = _constraint_grad(x, mu_star, k, cost_ledger=cost_ledger)
            ...
            residual = abs(_constraint_value(x, mu_star, k, cost_ledger=cost_ledger))
            diag["max_projection_residual"] = max(diag["max_projection_residual"], residual)
            diag["sum_projection_residual"] += residual
            diag["num_projection_residuals"] += 1
...
def run_rattle(key, mu_star, params, verbose=True, cost_ledger=None):
    ...
    mu_acc = 0
    x_acc = 0
    diag = _init_diag(x0, mu_star, k, T, cost_ledger=cost_ledger)
    ...
    for t in iters:
        ...
        momentum0, ok = _project_momentum(
            momentum0,
            _constraint_grad(x_cur, mu_star, k, cost_ledger=cost_ledger),
            grad_tol,
            cost_ledger=cost_ledger,
        )
        if not ok:
            ...
            residual_cur = abs(_constraint_value(x_cur, mu_star, k, cost_ledger=cost_ledger))
        ...
        residual_now = abs(_constraint_value(xs[t, :], mu_star, k, cost_ledger=cost_ledger))
