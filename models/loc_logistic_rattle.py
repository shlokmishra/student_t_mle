"""
Logistic location model baseline using a constrained HMC / RATTLE-style latent x update.

This keeps the current mu | x MH update comparable to models.loc_logistic while replacing
...
def _constraint_value(x, mu_star):
    return float(np.tanh(0.5 * (np.asarray(x) - mu_star)).sum())


def _constraint_grad(x, mu_star):
...
def _init_diag(x0, mu_star, proposals):
    residual0 = abs(_constraint_value(x0, mu_star))
    return {
...
def _project_to_manifold(x, mu_star, tol, max_iters, grad_tol):
    """
    Newton-style projection for a single smooth equality constraint.
    """
    x = np.asarray(x, dtype=float).copy()
    residual = abs(_constraint_value(x, mu_star))
    if residual <= tol:
        return x, True, 0, residual

    for it in range(1, max_iters + 1):
        grad = _constraint_grad(x, mu_star)
        denom = float(np.dot(grad, grad))
        if not np.isfinite(denom) or denom <= grad_tol:
            return x, False, it, residual
        step = _constraint_value(x, mu_star) / denom
        x = x - step * grad
        residual = abs(_constraint_value(x, mu_star))
        if residual <= tol:
            return x, True, it, residual
    return x, False, max_iters, residual
...
def _project_to_manifold_newton(
    ...
    """
    Newton-style projection with optional initial correction and backtracking line search.
    """
    x_trial = np.asarray(x, dtype=float).copy()
    residual = abs(_constraint_value(x_trial, mu_star))
    if residual <= tol:
        return x_trial, True, 0, residual

    if init_strategy == "linearized":
        grad0 = _constraint_grad(x_trial, mu_star)
        denom0 = float(np.dot(grad0, grad0))
        if np.isfinite(denom0) and denom0 > grad_tol:
            step0 = damping * _constraint_value(x_trial, mu_star) / denom0
    ...
    x_cur = x_trial
    for it in range(1, max_iters + 1):
        grad = _constraint_grad(x_cur, mu_star)
        denom = float(np.dot(grad, grad))
        if not np.isfinite(denom) or denom <= grad_tol:
            return x_cur, False, it, residual

        raw_step = _constraint_value(x_cur, mu_star) / denom
        alpha = float(damping)
        x_next = x_cur - alpha * raw_step * grad
        next_residual = abs(_constraint_value(x_next, mu_star))
...
def _grad_potential(x, mu):
    """
    Gradient of -log p(x | mu) for independent logistic observations.
    """
    return np.tanh(0.5 * (np.asarray(x) - mu))


def _potential_energy(x, mu):
    t = np.asarray(x) - mu
    return float(np.sum(t + 2.0 * np.logaddexp(0.0, -t)))


def _hamiltonian(x, momentum, mu):
    kinetic = 0.5 * float(np.dot(momentum, momentum))
    return _potential_energy(x, mu) + kinetic
...
def _rattle_trajectory(
    ...
    residual0 = abs(_constraint_value(x, mu_star))
    diag = {
        "position_projection_failures": 0,
        "momentum_projection_failures": 0,
    ...
    for _ in range(int(num_steps)):
        momentum = momentum - 0.5 * step_size * _grad_potential(x, mu)
        momentum, ok = _project_momentum(momentum, _constraint_grad(x, mu_star), grad_tol)
        if not ok:
        ...
        grad_new = _constraint_grad(x, mu_star)
...
def run_rattle(key, mu_star, params, verbose=True):
    """
    Two-step sampler:
    1. mu | x via the existing MH update from models.loc_logistic
    2. x | mu, MLE(x)=mu_star via a constrained HMC / RATTLE-style proposal
    """
    T = int(params["num_iterations_T"])
    n = int(params["n"])
    step_size = float(params.get("rattle_step_size", 0.15))
    num_steps = int(params.get("rattle_num_steps", 4))
    ...
    reverse_position_tol = float(params.get("rattle_reverse_position_tol", 2e-2))
    reverse_momentum_tol = float(params.get("rattle_reverse_momentum_tol", 2e-2))
    ...
    for t in iters:
        ...
        momentum0, ok = _project_momentum(momentum0, _constraint_grad(x_cur, mu_star), grad_tol)
        if not ok:
            xs[t, :] = x_cur
            diag["initial_momentum_projection_failures"] += 1
            diag["projection_failure_count"] += 1
            residual_cur = abs(_constraint_value(x_cur, mu_star))
        ...
        h0 = _hamiltonian(x_cur, momentum0, mu_new)
        ...
        h1 = _hamiltonian(x_prop, momentum_prop, mu_new)
        delta_h = h1 - h0
        ...
        residual_now = abs(_constraint_value(xs[t, :], mu_star))
...
def run_reversibility_test(
    ...
    """
    Sample representative (x, mu) states from the logistic RATTLE chain and test
    one forward/reverse constrained proposal from each with freshly sampled momentum.
    """
    states = sample_representative_states(
        key, mu_star, params,
        num_states=num_states, state_warmup=state_warmup, state_spacing=state_spacing, verbose=verbose,
    )
...
def sample_representative_states(
    ...
    """Sample representative (x, mu) states from a short logistic RATTLE chain."""
    params = dict(params)
...
def evaluate_reversibility_on_states(key, mu_star, params, states):
    ...
    for state in states:
        ...
        momentum0, ok0 = _project_momentum(momentum0, _constraint_grad(x_cur, mu_star), grad_tol)
        if not ok0:
            results.append({
        ...
        x_prop, p_prop, ok_fwd, _ = _rattle_trajectory(
            x_cur, momentum0, mu_cur, mu_star,
            step_size, num_steps, proj_tol, proj_max_iters, grad_tol,
            proj_damping, proj_line_search, proj_init_strategy,
        )
        if not ok_fwd:
            results.append({
                "state_index": int(state["state_index"]),
